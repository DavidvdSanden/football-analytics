...
matches_with_names = matches_with_names.sort_values(
    by="match_date",
    ascending=False
)
...
match_label = st.selectbox(
    "Match",
    matches_with_names["match_label"].tolist()
)

match_id = matches_with_names.loc[
    matches_with_names["match_label"] == match_label,
    "match_id"
].iloc[0]
...
home_team = teams[teams["team_id"] == selected_match_df['home_team_id'].values[0]]['team_name'].values[0]
away_team = teams[teams["team_id"] == selected_match_df['away_team_id'].values[0]]['team_name'].values[0]
